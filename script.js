    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; width: 320px;">
        ...
        <div style="padding: 12px 14px; background: white;">
            ...
            <table style="width:100%; border-collapse: collapse; font-size: 13px; color:#1e2b38; margin-bottom:8px;">
                ...
                <tr><td style="padding:3px 0; color:#8695a7;">REFCAT</td><td style="text-align:right; font-weight:600;">${p.REFCAT_x ?? ""}</td></tr>
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 4px 14px; font-size:12.5px; color:#1e2b38; background:#f7f9fa; border-radius:6px; padding:8px;">
                <div>Unid. autorizadas <b style="float:right;">${p.unidadesAutorizadas_Total ?? ""}</b></div>
                <div>Pue. autorizados <b style="float:right;">${p.puestosAutorizados_Total ?? ""}</b></div>
                <div>Unid. habilitadas <b style="float:right;">${p.unidadesHabilitadas_Total ?? ""}</b></div>
                <div>Pue. habilitados <b style="float:right;">${p.puestosHabilitados_Total ?? ""}</b></div>
                <div>Unid. sin func. <b style="float:right;">${p.unidadesNoFuncionamiento_Total ?? ""}</b></div>
                <div>Pue. sin func. <b style="float:right;">${p.puestosNoFuncionamiento_Total ?? ""}</b></div>
                <div style="color:#1e8449;">Unid. activas <b style="float:right;">${p.unidadesActivas_Total ?? ""}</b></div>
                <div style="color:#1e8449;">Pue. activos <b style="float:right;">${p.puestosActivos_Total ?? ""}</b></div>
            </div>
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 3px 14px; font-size:12px; color:#1e2b38;">
                <div>Ed. Infantil  <b style="float:right;">${p["unidadesAutorizadas_EDUCACIÓ INFANTIL SEGON CICLE"] ?? ""}</b></div>
                <div>Ed. Primaria <b style="float:right;">${p["unidadesAutorizadas_EDUCACIÓ PRIMÀRIA"] ?? ""}</b></div>
                <div>ESO <b style="float:right;">${p.unidadesAutorizadas_ESO ?? ""}</b></div>
                <div>Bachiller <b style="float:right;">${p.unidadesAutorizadas_BATXILLERAT ?? ""}</b></div>
            </div>
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 3px 14px; font-size:12px; color:#1e2b38;">
                <div>Ed. Infantil <b style="float:right;">${p.TOTAL_MATRICULACIONES_EI ?? ""}</b></div>
                <div>Ed. Primaria <b style="float:right;">${p.TOTAL_MATRICULACIONES_EP ?? ""}</b></div>
                <div>ESO <b style="float:right;">${p.TOTAL_MATRICULACIONES_ESO ?? ""}</b></div>
                <div>Bachiller <b style="float:right;">${p.TOTAL_MATRICULACIONES_BACH ?? ""}</b></div>
            </div>
